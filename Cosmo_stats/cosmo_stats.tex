...
There is also a figure of merit used by the Dark Energy Task Force \cite{detf} which is defined to be the reciprocal of the area in the plane enclosing the $95\%$ confidence limit of two parameters. This figure of merit can be used to see how different experiments break degeneracies using it as an additional parameter.
...
\begin{equation}
P(\theta|D,H)=\frac{1}{P(D|H)}\int P(\theta|\alpha,H)P(\alpha|D, H)d\alpha
\end{equation}
...
\end{equation}
Here, the quantity inside square brackets is the marginalized likelihood over hyperparameters $L(D;\theta,H)$, and then we can identify the quantity inside the integration as the individual likelihood $L(D_k;\theta,\alpha,H)$, for every $\alpha_k$ and data set $D_k$; $P(D|H)$ is the evidence and, in a typical parameter inference procedure, works as a normalized function, i.e. $P(D|H)=\int P(\theta,H)L(D;\theta,H)$. Notice that, by considering $P(\alpha_k)=\delta(\alpha_k-1)$, we rely on the standard approach, where no hyperparameters are used.  
...
The basic idea of the Gibbs sampling algorithm \cite{gibbs} is to split the multidimensional $\theta$ into blocks and sample each block separately, conditional on the most recent values of the other blocks. It basically breaks a high-dimensional problem into low-dimensional problems.

The algorithm reads as follows:

\begin{enumerate}
\item $\theta$ consists of $k$ blocks $\theta_1, ..., \theta_k$; at step $i$
\item Draw $\theta^{i+1}_{1}$ from $p(\theta_1|\theta^{i}_{2}, ...,\theta^{i}_{k})$
\item Draw $\theta^{i+1}_{2}$ from $p(\theta_2|\theta^{i+1}_{1}, \theta^{i}_{3}, ...,\theta^{i}_{k})$\
\item ...
\item Repeat the above steps for the wished iterations with $i \rightarrow i+1$.
\end{enumerate}
The distribution $p(\theta_1|\theta_{2}, ...,\theta_{k})=\frac{p(\theta_1, ..., \theta_k)}{p(\theta_2, ..., \theta_k)}$ is known as the \textit{full conditional distribution of $\theta_1$}. 
...
If now we consider that there are values for $a$ and $b$ for which our data fits better, then from \eqref{GLik} we can write our likelihood as
\begin{equation}
...
\captionof{figure}{\footnotesize{Stright-lines for Case 2 inferred by our procedure confronted with data.}}
\label{contour_4}
\end{minipage}
...
%When this results and CMB data are combined we obtain \cite{parametros}:
%
%\begin{equation}
...
%Figure \ref{CMBPlanck} shows the last CMB anisotropies picture obtained by Planck this year
%\begin{figure}[h]
%	\centering
...
\bibitem{COBE}{\em COBE}\url{https://science.nasa.gov/missions/cobe}

\bibitem{WMAP}{\em WMAP}\url{https://www.nasa.gov/pdf/594754main_Citrin_Liz.pdf}
...
\bibitem{retos} Vázquez-González A., Matos T., Rev. Mex. Fís. E 54 (2) (2008) 193–202.
...
\bibitem{detf} Albrecht A. et al., Report of the Dark Energy Task Force, June 2006.

\bibitem{gibbs} Geman S., Geman D., ``Stochastic Relaxation, Gibbs Distributions, and the Bayesian Restoriation of Images''`, 1984.
